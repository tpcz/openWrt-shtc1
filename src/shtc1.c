 ...
 *
 * Data sheet available from sensirion website
 * 
 */
...
static ssize_t shtc1_show_identification(struct device *dev,
    ...
    struct i2c_client *client = to_i2c_client(dev);
    struct shtc1_data *data = i2c_get_clientdata(client);
    int data_len = sizeof (data->identification);

    buf_ptr = buf;
    for (i = 0; i < data_len; ++i)
        buf_ptr += sprintf(buf_ptr, "%02X", data->identification[i]);

    sprintf(buf_ptr, "\n");

    return (ssize_t) data_len * 2 + 1;
}
...
static SENSOR_DEVICE_ATTR(identification, S_IRUGO,
        shtc1_show_identification, NULL, 0);
...
static struct attribute *shtc1_attributes[] = {
    &sensor_dev_attr_identification.dev_attr.attr,
...
static int shtc1_probe(struct i2c_client *client,
    ...
    data = (struct shtc1_data*) devm_kzalloc(&client->dev,
            sizeof (struct shtc1_data), GFP_KERNEL);
    ...
    err = i2c_master_recv(client, data->identification,
            sizeof (data->identification));
    ...
    if ((data->identification[1] & shtc1_id_reg_mask) != shtc1_id_reg_content) {
